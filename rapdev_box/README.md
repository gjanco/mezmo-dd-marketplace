...
1. Run the following command to remove the integration:
    ...
    - Linux: `sudo -u dd-agent datadog-agent integration remove datadog-rapdev_box`

    - Windows: `“C:\Program Files\Datadog\Datadog Agent\bin\agent.exe" integration remove datadog-rapdev_box”`
...
- If you plan to reinstall or need to keep the config files:
    - Navigate to your Agent's `conf.d` directory and locate the `rapdev_box.d` folder to access the YAML configs. **NOTE**: These files contain sensitive information such as user/password info and API keys.
    
- If you plan to fully uninstall with config removal:
    - Navigate to your Agent's `conf.d` directory, and remove the `rapdev_box.d` folder.
