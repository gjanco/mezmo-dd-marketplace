...
2. Download the HP-UX Agent install [package](http://rapdev-files.s3.amazonaws.com/hpux/DatadogAgent-1.5.4.tgz).
```sh
mkdir -p /tmp/datadog-agent-install
cd /tmp/datadog-agent-install
/usr/local/bin/curl -o DatadogAgent-1.5.4.tgz http://rapdev-files.s3.amazonaws.com/hpux/DatadogAgent-1.5.4.tgz 
```

3. Install the HP-UX Agent. You can run the `install.sh` script to update an existing HP-UX Agent install.
```sh
cd /tmp/datadog-agent-install
gunzip -c DatadogAgent-1.5.4.tgz | tar -xvf -
./install.sh 
```
