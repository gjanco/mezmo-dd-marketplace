...
class CDSDellEMCNdHelper(object):
    ...
    def nd_temperature_details_ot(self, response_json):
        """Method to get Temperature Details Over Time."""
        metric_prefix = "{}temperature_details_over_time".format(self.nd_prefix)
        metric_list = list()
        ...
        for statistics in response_json.get("stats", []):
            ...
            if "node.sensor.temp.celsius." in statistics.get("key", ""):
                sensor_id = statistics.get("key").split(".")[4]
                disk_details[node_id].update({"sensor-%s" % sensor_id: metric_value if metric_value is not None else 0})
            ...
            for key, metric_value in disk_usage.items():
                metric_list.append(
                    [
                        ...
                        [
                            INTEGRATION_PREFIX + "node:node-%s" % node_id,
                            INTEGRATION_PREFIX + "sensor:%s" % key,
                        ],
                    ]
                )
        return metric_list

    def nd_power_details_over_time(self, response_json):
        """Method to get Power Details Over Time."""
        metric_prefix = "{}power_details_over_time".format(self.nd_prefix)
        metric_list = list()
