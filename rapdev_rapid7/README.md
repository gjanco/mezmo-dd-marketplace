...
1. Run the following command to remove the integration:
    ...
    - Linux: `sudo -u dd-agent datadog-agent integration remove datadog-rapdev_rapid7`

    - Windows: `“C:\Program Files\Datadog\Datadog Agent\bin\agent.exe" integration remove datadog-rapdev_rapid7”`
...
- If you plan to reinstall or need to keep the config files:
    - Navigate to your Agent's `conf.d` directory and locate the `rapdev_rapid7.d` folder to access the YAML configs. **NOTE**: These files contain sensitive information such as API keys.
    
- If you plan to fully uninstall with config removal:
    - Navigate to your Agent's `conf.d` directory, and remove the `rapdev_rapid7.d` folder.

Rapid7 Cleanup:
- As a best practice, remove any associated API keys created exclusively for this integration. For more details, reference the Installation section.
