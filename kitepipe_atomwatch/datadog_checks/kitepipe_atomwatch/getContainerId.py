...
def getContainerId(self, install_dir):
    ...
    # INPUTS:
    # install_dir...        This plus /conf/ is where the container.id file
    #                       lives
    #
    # OUTPUTS:
    ...
    try:
        ...
        msg = (
            'Configuration error.  Failed to get container id from '
            + 'installation directory `'
            + install_dir
            + '`. Error message: '
            + str(e)
            + '\n\nCheck if Boomi is installed in this directory and '
            + 'if the directory is readable by dd-agent user.'
        )
