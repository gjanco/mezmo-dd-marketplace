...
def queryBoomiEventsApi(
    ...
    #
    # OUTPUTS:
    # arr_events...    Array of Event objects just as they came from Boomi
    #                  API but assembled into array.
    ...
    sEventsApiUrl = boomi_api_url + "/api/rest/v1/"
    sEventsApiUrl += boomi_account_id + "/Event/query"

    # Get first page of Boomi Events API response
    fn = getBoomiEventsApiResponsePage
    ar1 = boomi_api_userid
    ar2 = boomi_api_token
    ar3 = sEventsApiUrl
    eventResponse = fn(self, ar1, ar2, ar3, apiRequest, cluster_node_id)

    # If we reached here, we got a first page.
    logmsg = "Total count of Events in Query Result: "
    logmsg += str(eventResponse['numberOfResults'])
...
def getBoomiEventsApiResponsePage(self, ar1, ar2, ar3, ar4, cluster_node_id):
    ...
    self.count(st, 1, tags=["boomiapicalltype:Event"])
...
'''.format(
    ...
    if 'numberOfResults' in response_json and 'result' in response_json:
        ...
        errmsg = '''\
Boomi API Event query response JSON content was not as expected.
Missing 'numberOfResults' or 'result'.
Boomi Event API URL: {}
