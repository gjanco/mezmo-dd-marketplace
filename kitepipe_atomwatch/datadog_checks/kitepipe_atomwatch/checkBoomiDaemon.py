...
def checkBoomiDaemon(self, instance, boomi_install_dir):
    ...
    # boomi_install_dir...  If running on Linux, this proc will look for
    #                       boomi_install_dir as a substring of `ps -ef`
    #                       to judge whether Boomi daemon is running on \
    #                       this box.
    #
    # OUTPUTS:
    ...
    try:
        if our_os == "Windows":
            ...
            for windows_service in psutil.win_service_iter():
                ...
                if found:
                    # We assume we found a Windows service running a Boomi
                    # runtime.
                    # Get the status
                    ...
                    # By default we say the check result is CRITICAL
                    ar = self.CRITICAL
                    # But if service is running we say it is OK
            ...
            st = ' (' + boomi_install_dir + '.*?([^/ ]+)) '
            found = re.search(st, all_ps)
            if found:
        ...
        ################
        # Log a warning
        msg = 'Failure when interrogating OS to see if Boomi '
        msg += 'daemon/service is running: ' + str(e)
        self.warning(msg)
        # Output the service_check to show Boomi daemon NOT running
        self.service_check(
            __NAMESPACE__ + 'boomi_daemon_running',
