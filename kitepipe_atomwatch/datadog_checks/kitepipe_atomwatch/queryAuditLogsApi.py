...
def queryBoomiAuditLogsApi(
    ...
    # Calls Boomi AuditLogs API, pages over results, returns array
    # of AuditLogs.
    #
    # INPUTS:
    ...
    # startDateTime...     TZ-aware datetime with UTC TZ, precise to the
    #                      second but not more granular than that.
    # endDateTime...       Corresponds to startDateTime.
    # boomi_account_id...  Boomi AtomSphere Platform account ID; used in
    #                      making API calls.
    #
    # OUTPUTS:
    # arr_auditlogs...    Array of AuditLog objects just as they came from
    #                     Boomi API but assembled into array.
    ...
    # Prepare the data to be used as request JSON input to Boomi AuditLogs
    # API for all auditlogs in date range.
    st1 = startDateTime.isoformat()[0:19] + "Z"
    st2 = endDateTime.isoformat()[0:19] + "Z"
    apiRequest = {
        "QueryFilter": {
            "expression": {
                "argument": [st1, st2],
    ...
    sAuditLogsApiUrl = boomi_api_url + "/api/rest/v1/"
    sAuditLogsApiUrl += boomi_account_id + "/AuditLog/query"

    # Get first page of Boomi AuditLogs API response
    ar1 = boomi_api_userid
    ar2 = boomi_api_token
    ar3 = sAuditLogsApiUrl
    fn = getBoomiAuditLogsApiResponsePage
    auditlogResponse = fn(self, ar1, ar2, ar3, apiRequest, cluster_node_id)

    # If we reached here, we got a first page.
    logmsg = "Total count of AuditLogs in Query Result: "
    logmsg += str(auditlogResponse['numberOfResults'])
...
def getBoomiAuditLogsApiResponsePage(self, ar1, ar2, ar3, ar4, ar5):
    ...
    self.count(st, 1, tags=["boomiapicalltype:AuditLog"])
...
'''.format(
    ...
    if 'numberOfResults' in response_json:
        ...
        errmsg = '''\
Boomi API AuditLog query response JSON content was not as expected.
Missing 'numberOfResults'.
