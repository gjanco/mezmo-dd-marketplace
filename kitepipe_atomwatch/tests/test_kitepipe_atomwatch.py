...
from datadog_checks.kitepipe_atomwatch import AtomWatchCheck

# import local code
__NAMESPACE__ = 'kitepipe.atomwatch.'
LAST_END_DATETIME_FILENAME = "kitepipe-atomwatch-last-end-datetime.txt"
    ...
    class Mock_get_response:
        ...
        def json(self):
            ...
            if found:
                ...
                stgd_js_fname = "boomi_atom_get.json"

            # Read the Atom Get data from disk.
            f_name = os.path.join(get_here(), 'http', stgd_js_fname)
...
def mock_post(boomi_api_url, json, headers, auth):
    ...
    class Mock_post_response:
        ...
        def json(self):
            ...
            # Inspect the passed URL.
            # Does it try to do a query of Execution Records
            # (scenarios 1 and 3)?
            ...
            if found:
                ...
                if found:
                    # Caller wants page 1 of ExecutionRecord query
                    # by timestamp.
                    stgd_js_fname = "boomi_executions_by_timestamp_pg_1.json"
                found = re.search("executionId", s_json)
                if found:
                    # Caller wants page 1 of ExecutionRecord query by
                    # execution ID.
                    stgd_js_fname = "boomi_executions_by_execid_pg_1.json"

            # Inspect the passed URL.
            # Does it try to do a queryMore of Execution Records
            # (scenarios 2 and 4)?
            ...
            if found:
                ...
                if json == "tokenA":
                    # Caller wants page 2 of ExecutionRecord query
                    # by timestamp.
                    stgd_js_fname = "boomi_executions_by_timestamp_pg_2.json"
                if json == "tokenB":
                    # Caller wants page 2 of ExecutionRecord query
                    # by execution ID.
                    stgd_js_fname = "boomi_executions_by_execid_pg_2.json"
            ...
            if found:
                ...
                stgd_js_fname = "boomi_auditlogs_by_timestamp_pg_1.json"
            ...
            if found:
                ...
                if json == "tokenC":
                    ...
                    stgd_js_fname = "boomi_auditlogs_by_timestamp_pg_2.json"
            ...
            if found:
                ...
                stgd_js_fname = "boomi_events_by_timestamp_pg_1.json"
            ...
            if found:
                ...
                if json == "tokenD":
                    ...
                    stgd_js_fname = "boomi_events_by_timestamp_pg_2.json"
            ...
            if found:
                ...
                stgd_js_fname = "boomi_atom_query.json"
            ...
            if found:
                ...
                stgd_js_fname = "datadog_log_intake.json"
...
def test_config():
    ...
    # datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml
    instance = {}
    c = AtomWatchCheck('kitepipe_atomwatch', {}, [instance])
    ...
    try:
        ...
        assert (
            e.__str__()
            == "Configuration error.  Value of dd_api_key is missing. Please \
edit datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to supply this value."
        )

    # BAD CONFIG: Missing the Boomi API userid while boomi_api_url present
    try:
        myjs = '{"dd_api_key": "Q", "boomi_api_url": '
        myjs += '"https://api.boomi.com", "boomi_api_token": "foo"}'
        c.check(json_import.loads(myjs))
    except Exception as e:
        assert (
            e.__str__()
            == 'Configuration error.  Value of boomi_api_url present but \
boomi_api_userid is missing. Populate boomi_api_url only on one server so \
that only one set of Boomi API calls is made. Please edit \
datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to supply missing value.'
        )

    # BAD CONFIG: Missing the the Boomi API token while boomi_api_url present
    try:
        myjs = '{"dd_api_key": "some DD key", '
        myjs += ' "boomi_api_url": "https://api.boomi.com", '
        myjs += '"boomi_api_userid": "foo"}'
        c.check(json_import.loads(myjs))
    except Exception as e:
        assert (
            e.__str__()
            == 'Configuration error.  Value of boomi_api_url present but \
boomi_api_token is missing. Populate boomi_api_url only on one server so \
that only one set of Boomi API calls is made. Please edit \
datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to supply missing value.'
        )
    ...
    try:
        ...
        assert (
            e.__str__()
            == 'Configuration error.  Value of boomi_api_url present but \
boomi_account_id is missing. Populate boomi_api_url only on one server so \
that only one set of Boomi API calls is made. Please edit \
datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to supply missing value.'
        )
    ...
    try:
        ...
        assert (
            e.__str__()
            == "Configuration error.  Value of \
boomi_atom_or_molecule_install_dir is missing. Please edit \
datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to supply this value."
        )
...
Please edit datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to supply \
this value."
        )
    ...
    try:
        ...
        assert (
            e.__str__()
            == 'Configuration error.  Value of min_boomi_api_interval is \
missing. Please edit datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to \
supply this value. Value must be 300 or greater, which lets the Datadog Agent \
call Boomi Platform APIs every 5 minutes or less frequently.  It is not \
supported to call Boomi Platform APIs more frequently than every 5 minutes.'
        )
...
an integer. Please edit datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml \
to correct this. Value must be 300 or greater, which lets the Datadog Agent \
call Boomi Platform APIs every 5 minutes or less frequently.  It is not \
supported to call Boomi Platform APIs more frequently than every 5 minutes.'
        )
    ...
    try:
        ...
        assert (
            e.__str__()
            == 'Configuration error.  Value of min_boomi_api_interval is \
too low. Please edit datadog-agent/conf.d/kitepipe_atomwatch.d/conf.yaml to \
increase this value. Value must be 300 or greater, which lets the Datadog \
Agent call Boomi Platform APIs every 5 minutes or less frequently.  It is \
not supported to call Boomi Platform APIs more frequently than \
every 5 minutes.'
        )
    ...
    try:
        ...
        assert (
            e.__str__()
            == 'Configuration error.  Both boomi_api_gateway_node_id and \
boomi_molecule_node_id are populated.  Only one of these must be populated, \
or neither.'
        )
    ...
    try:
        ...
        assert (
            e.__str__()
            == 'Configuration error.  boomi_api_gateway_node_id is populated \
but boomi_api_gateway_install_dir is blank.  If this server is an API Gateway \
node, please populate both of these values.'
        )
...
def test_service_check(aggregator, instance):
    ...
    bagid = boomi_api_gateway_install_dir
    filepath = os.path.join(baomid, "conf", "container.id")
    f = open(filepath, "w")
    f.write("com.boomi.container.id=atom_container_id\n")
    f.write("com.boomi.container.account=kitepipedev-BHRF6P")
    f.close()
    filepath = os.path.join(bagid, "conf", "container.id")
    f = open(filepath, "w")
    f.write("com.boomi.container.id=api_gateway_container_id\n")
    f.write("com.boomi.container.account=kitepipedev-BHRF6P")
    f.close()
    filepath = os.path.join(baomid, "work", LAST_END_DATETIME_FILENAME)
    f = open(filepath, "w")
    f.write("2022-12-31T19:28:03")
    f.close()
    filepath = os.path.join(baomid, "bin", "views", "node.10_10_0_2.dat")
    f = open(filepath, "w")
    f.write("nothing here")
    f.close()
    filepath = os.path.join(baomid, "bin", "views", "node.10_10_0_3.dat")
    f = open(filepath, "w")
    f.write("nothing here")
    f.close()
    filepath = os.path.join(bagid, "bin", "views", "node.10_10_1_2.dat")
    f = open(filepath, "w")
    f.write("nothing here\nalso nothing here\n")
    f.write("problem=ROLLING_RESTART_MULTIPLE_HEAD_NODES\nmaybe more stuff")
    f.close()
    filepath = os.path.join(bagid, "bin", "views", "node.10_10_1_3.dat")
    f = open(filepath, "w")
    f.write("nothing here")
    f.close()
    ...
    myst = __NAMESPACE__ + 'config_validated'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'datetime_range_set_up'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'boomi_api_calls_permitted'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'persisted_end_datetime'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'boomi_daemon_running'
    aggregator.assert_service_check(myst, AtomWatchCheck.CRITICAL)
    ...
    myst = __NAMESPACE__ + 'skipping_boomi_api_calls'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'skipped_molecule_view_file'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'skipped_api_gateway_view_file'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    ...
    #   Stash some execution IDs so that Agent will try to query
    #    Boomi API for them
    c.write_persistent_cache("in_progress_execution_ids", "execID_1,execID_2")

    # Don't actually call Boomi API... use mock instead.
    myst = 'tests.test_kitepipe_atomwatch.AtomWatchCheck.http'
    with mock.patch(myst) as mock_http:
        # Here we supply our own "post()" and "get()" methods that
        # will read from disk instead of calling Boomi API.
    ...
    myst = __NAMESPACE__ + 'queried_boomi_executions'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'found_boomi_executions'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'submitted_execution_metrics'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'submitted_execution_logs'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'queried_boomi_auditlogs'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'found_boomi_auditlogs'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'submitted_auditlog_events'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'queried_boomi_events'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'found_boomi_events'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'runtime_reported_online'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'runtime_reported_online'
    aggregator.assert_service_check(myst, AtomWatchCheck.CRITICAL)
    myst = __NAMESPACE__ + 'submitted_boomievent_events'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'checked_molecule_view_file'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    myst = __NAMESPACE__ + 'longrunning_execution_failed_metric_submission'
    aggregator.assert_service_check(myst, AtomWatchCheck.WARNING)
    ...
    myst = __NAMESPACE__ + 'checked_api_gateway_view_file'
    aggregator.assert_service_check(myst, AtomWatchCheck.OK)
    ...
    myst = __NAMESPACE__ + 'used_default_last_end_datetime'
    aggregator.assert_service_check(myst, AtomWatchCheck.WARNING)

    # Verify metrics
    m = "kitepipe.atomwatch."
    aggregator.assert_metric(m + "boomi_api_calls_attempted")
    aggregator.assert_metric(m + "execution.measure.duration")
    aggregator.assert_metric(m + "execution.measure.inboundDocumentCount")
    aggregator.assert_metric(m + "execution.measure.inboundDocumentSize")
    aggregator.assert_metric(m + "execution.measure.inboundErrorDocumentCount")
    aggregator.assert_metric(m + "execution.measure.outboundDocumentCount")
    aggregator.assert_metric(m + "execution.measure.outboundDocumentSize")
    m2 = "execution."
    aggregator.assert_metric(m + m2 + "measure.inProgressExecutionDuration")
    aggregator.assert_metric(m + "execution.status")
    aggregator.assert_metric(m + "view_file_age_seconds")
    aggregator.assert_metric(m + "view_file_exist")
    aggregator.assert_metric(m + "view_file_problem")
    aggregator.assert_metric(m + "integration_completed")
    aggregator.assert_metric("datadog.marketplace.kitepipe.atomwatch")
